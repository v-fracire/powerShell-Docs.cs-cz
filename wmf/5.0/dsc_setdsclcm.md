...
ms.locfileid: "34187057"
---
# <a name="set-dsclocalconfigurationmanager-cmdlet-supports--force-parameter"></a><span data-ttu-id="e5cb3-102">Podporuje rutiny Set-DscLocalConfigurationManager - force parametr</span><span class="sxs-lookup"><span data-stu-id="e5cb3-102">Set-DscLocalConfigurationManager cmdlet supports -force parameter</span></span>

<span data-ttu-id="e5cb3-103">Rutině Set-DscLocalConfigurationManager jsme doplnili podporu pro nový parametr.</span><span class="sxs-lookup"><span data-stu-id="e5cb3-103">We have added a support for new parameter to Set-DscLocalConfigurationManager cmdlet.</span></span> <span data-ttu-id="e5cb3-104">To vám umožní uživatelům nepodmíněně resetování konfigurace meta na počítači, v případě, že jiné operace, jako je kontrola konzistence běží v pozadí, jako by to způsobilo všechny běžící operace zastavení.</span><span class="sxs-lookup"><span data-stu-id="e5cb3-104">This will allow the user to reset meta configuration on machine deterministically when other operations like consistency check are running in background as it will cause all running operations to be stopped.</span></span>

<span data-ttu-id="e5cb3-105">Možnosti vypadá takto při pokusu o nastavení konfigurace meta bez – Force parametru.</span><span class="sxs-lookup"><span data-stu-id="e5cb3-105">The experience looks like this when trying to set meta configuration without –Force parameter.</span></span>
```powershell
...
<span data-ttu-id="e5cb3-106">Když používáme – vynutit úspěšně aktualizace konfigurace meta systému podle zrušení aktuální běžící operace na počítači.</span><span class="sxs-lookup"><span data-stu-id="e5cb3-106">When we use –force it successfully updates the meta configuration on system by canceling the current running operation on the machine.</span></span>
```powershell
