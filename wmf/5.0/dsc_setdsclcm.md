...
---
# <a name="set-dsclocalconfigurationmanager-cmdlet-supports--force-parameter"></a>Podporuje rutiny Set-DscLocalConfigurationManager - force parametr

Rutině Set-DscLocalConfigurationManager jsme doplnili podporu pro nový parametr. To vám umožní uživatelům nepodmíněně resetování konfigurace meta na počítači, v případě, že jiné operace, jako je kontrola konzistence běží v pozadí, jako by to způsobilo všechny běžící operace zastavení.

Možnosti vypadá takto při pokusu o nastavení konfigurace meta bez – Force parametru.
```powershell
...
Když používáme – vynutit úspěšně aktualizace konfigurace meta systému podle zrušení aktuální běžící operace na počítači.
```powershell
