...
Následující konfigurace se používá **souboru** prostředek pro vytvoření sdílené složky v adresáři a **xSmbShare** prostředků nastavení sdílené složky SMB:
...
Konfigurace vytvoří adresář `C:\DscSmbShare`, pokud ještě neexistuje a potom použije tento adresář jako sdílené složky protokolu SMB. **FullAccess** by měla být věnována libovolný účet, který se musí zapisovat nebo odstranit ze sdílené složky. **Oprávnění ke čtení** se musí předávat do žádného uzlu klienta, které si mohli služby konfigurace a prostředky DSC ze sdílené složky.

> [!NOTE]
> DSC jako systémový účet ve výchozím nastavení spustí, takže daného počítače, musí mít přístup ke sdílené složce.
...
Každý modul prostředků musí být metoda ZIP a s názvem podle následujícího vzoru `{Module Name}_{Module Version}.zip`. Například modul s názvem xWebAdminstration s verzí modulu 3.1.2.0 by se pojmenoval "xWebAdministration_3.2.1.0.zip". Každá verze modulu musí být součástí jedné zip soubor. Samostatné verze modulu jako soubor zip nejsou podporovány. Před zabalení moduly prostředků DSC pro použití se serverem o přijetí změn, je potřeba proveďte malou změnu do struktury adresářů.

Je výchozí formát pro moduly obsahující prostředek DSC ve WMF 5.0 `{Module Folder}\{Module Version}\DscResources\{DSC Resource Folder}\`.

Před balení do serveru vyžádané replikace jednoduše odebrat `{Module version}` složku, cesta bude `{Module Folder}\DscResources\{DSC Resource Folder}\`. Díky této změně složky zip, jak je popsáno výše a umístí tyto souboru zip do sdílené složky SMB.
...
Speciální díky jednotlivce následující:
