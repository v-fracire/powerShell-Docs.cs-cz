...
4. Spusťte následující příkazy prostředí PowerShell pro kompilaci dokument MOF (informace o kompilaci konfigurace DSC najdete v tématu [konfigurací DSC](../configurations/configurations.md):
...
5. Tím se vytvoří `localhost.mof` souboru do nové složky s názvem `SampleIISInstall`.
   Přejmenování a přesunutí tohoto souboru do správného umístění na virtuálním pevném disku jako `Pending.mof` pomocí [přesunout položku](/powershell/module/microsoft.powershell.management/move-item) rutiny. Příklad:
...
Můžete to ověřit pomocí volání [Get-WindowsFeature](/powershell/module/servermanager/get-windowsfeature) rutiny.

## <a name="inject-a-dsc-metaconfiguration-into-a-vhd"></a>Vložit DSC metaconfiguration do virtuálního pevného disku

Můžete také nakonfigurovat počítač k o přijetí změn konfigurace při počáteční spuštění-vložením metaconfiguration (viz [konfigurace místní Configuration Manageru (LCM)](../managing-nodes/metaConfig.md)) do virtuálního pevného disku jako jeho `MetaConfig.mof` souboru.
...
Informace o nastavení serveru vyžádané replikace DSC najdete v tématu [nastavení webového serveru vyžádané replikace DSC](../pull-server/pullServer.md).
...
2. [Nastavení webového serveru vyžádané replikace DSC](../pull-server/pullServer.md)a uložit **SampleIISInistall** konfigurace do příslušné složky.
...
5. Spusťte následující příkazy prostředí PowerShell pro kompilaci dokument MOF metaconfiguration (informace o kompilaci konfigurace DSC najdete v tématu [konfigurací DSC](../configurations/configurations.md):
...
6. Tím se vytvoří `localhost.meta.mof` souboru do nové složky s názvem `PullClientBootstrap`.
   Přejmenování a přesunutí tohoto souboru do správného umístění na virtuálním pevném disku jako `MetaConfig.mof` pomocí [přesunout položku](/powershell/module/microsoft.powershell.management/move-item) rutiny.
...
[Konfigurace Local Configuration Manageru (LCM)](../managing-nodes/metaConfig.md)

[Nastavení webového serveru vyžádané replikace DSC](../pull-server/pullServer.md)
