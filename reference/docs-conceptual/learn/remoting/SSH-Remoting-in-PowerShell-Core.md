...
SSH musí být nainstalován ve všech počítačích. Instalace klienta SSH (`ssh.exe`) a serveru (`sshd.exe`), který umožňuje vzdálený do a z počítače. OpenSSH pro Windows je teď ve Windows 10 sestavení 1809 a Windows Server 2019. Další informace najdete v tématu [OpenSSH pro Windows](/windows-server/administration/openssh/openssh_overview). Pro Linux instalace SSH (včetně server sshd) pro vaši platformu. Také musíte nainstalovat PowerShell Core z Githubu, které chcete tuto funkci vzdálenou komunikaci SSH. SSH server musí nakonfigurovat, aby vytvořila podsystému SSH k hostování prostředí PowerShell procesu ve vzdáleném počítači. Musíte také povolit heslo nebo nastavit ověřování na základě klíče.

## <a name="set-up-on-windows-machine"></a>Nastavení na počítači s Windows

1. Nainstalujte nejnovější verzi [PowerShell Core pro Windows](../../install/installing-powershell-core-on-windows.md#msi)
...
2. Nainstalujte nejnovější Win32 OpenSSH. Pokyny k instalaci, naleznete v tématu [instalace OpenSSH](/windows-server/administration/openssh/openssh_install_firstuse).
3. Upravit `sshd_config` se nachází v `%ProgramData%\ssh`.
...
1. Nainstalujte nejnovější [PowerShell Core pro Linux](../../install/installing-powershell-core-on-linux.md#ubuntu-1404) sestavení z Githubu
...
1. Nainstalujte nejnovější [PowerShell Core pro MacOS](../../install/installing-powershell-core-on-macos.md) sestavení
...
[PowerShell Core pro Windows](../../install/installing-powershell-core-on-windows.md#msi)

[PowerShell Core pro Linux](../../install/installing-powershell-core-on-linux.md#ubuntu-1404)

[PowerShell Core pro MacOS](../../install/installing-powershell-core-on-macos.md)

[OpenSSH pro Windows](/windows-server/administration/openssh/openssh_overview)
