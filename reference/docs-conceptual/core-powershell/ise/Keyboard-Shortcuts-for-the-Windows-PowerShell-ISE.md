...
---
# <a name="keyboard-shortcuts-for-the-windows-powershell-ise"></a>Klávesové zkratky prostředí Windows PowerShell ISE

Pomocí následujících klávesových zkratek k provádění akcí ve Windows Powershellu® integrovaném skriptovacím prostředí (ISE). Windows PowerShell ISE je k dispozici jako součást systému Windows Server a Windows klientské operační systémy, ale může také nainstalovat v některých starších operačních systémech Windows jako součást [balíček ke stažení pro Windows Management Framework 4.0](https://go.microsoft.com/fwlink/?LinkID=293881).
...
|----------|----------------------|----------|
|**Pomoc**|F1|Podokno skriptu **důležité:** můžete určit, že nápovědy klávesy F1 pochází z na stránkách TechNet Library na webu nebo stáhli nápovědy (viz Update-Help). K výběru, klikněte na tlačítko **nástroje**, **možnosti**, pak na **obecné nastavení**kartu nebo zrušte zaškrtnutí **nahrazujícím obsah místní nápovědy online obsahu.**|
|**Kopírování**|Ctrl + C|Podokno skriptu, příkazovém podokně výstup podokno|
|**Vyjmout**|CTRL + X|Podokno skriptu, příkazovém podokně|
|**Rozbalit nebo sbalit osnovy**|CTRL+M|Podokno skriptu|
...
|**Najít odpovídající závorku**|CTRL +]|Podokno skriptu|
|**Vložit**|CTRL + V|Podokno skriptu, příkazovém podokně|
|**Znovu:**|CTRL + Y|Podokno skriptu, příkazovém podokně|
|**Nahraďte ve skriptu**|CTRL + H|Podokno skriptu|
|**Uložení**|CTRL+S|Podokno skriptu|
|**Vybrat vše**|CTRL + A|Podokno skriptu, příkazovém podokně výstup podokno|
|**Zobrazení fragmentů**|CTRL+J|Podokno skriptu, příkazovém podokně|
|**Vrácení zpět**|CTRL + Z|Podokno skriptu, příkazovém podokně|
...
|**Karta** (pro další skriptu)|CTRL + TAB **Poznámka:** kartu a další skript funguje pouze v případě, že máte do jedné karty Windows Powershellu spustit, nebo když máte více než jedné karty Windows Powershellu otevřete, ale je zaměřena tak, v podokně skriptu.|
|**Karta** (do předchozího skriptu)|CTRL + SHIFT + TAB **Poznámka:** kartu předchozí skript funguje, pokud máte pouze jeden otevřít kartu prostředí Windows PowerShell, nebo pokud máte více než jedné karty Windows Powershellu otevřete a fokus se v podokně skriptu.|
...
|Akce|Klávesové zkratky|
|----------|---------------------|
|**Přejděte na příkaz (v2) nebo konzoly (verze 3 a novější) podokno**|CTRL + D|
|**Přejděte do podokna výstup (pouze verze 2)**|CTRL + SHIFT + O|
...
> [!NOTE]
> Můžete také použít klávesové zkratky, určené pro konzolu prostředí Windows PowerShell při ladění skriptů v prostředí Windows PowerShell ISE. Pokud chcete použít tyto klávesové zkratky, musíte zadejte klávesovou zkratku v podokně příkaz a stiskněte klávesu ENTER.
...
|**Předchozí karta prostředí PowerShell**|CTRL+SHIFT+TAB. Tato zkratka funguje pouze v případě, že žádné soubory nejsou na libovolné kartě Windows Powershellu otevřené.|
|**Další karta prostředí Windows PowerShell**|CTRL + TAB. Tato zkratka funguje pouze v případě, že žádné soubory nejsou na libovolné kartě Windows Powershellu otevřené.|
...
- [Prostředí PowerShell Magazine: Úplný seznam klávesových zkratek Windows PowerShell ISE](https://www.powershellmagazine.com/2013/01/29/the-complete-list-of-powershell-ise-3-0-keyboard-shortcuts/)